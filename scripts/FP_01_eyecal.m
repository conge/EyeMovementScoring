...
[a1,trigger_time,trigger]=textread([respath,subid,'fp_trigger.txt'],'%s%f%s');
...
%x_right=zeros(WindowLength);
 ...
 %% specifying one eye to score.
    ...
    elseif eye==2, 
        ...
        maxfix=max(x_right);
        minfix=min(x_right);        
    else error(['(1=Left Eye; 2=Right). You just entered "',num2str(eye),'". Please restart FP_01_eyecal.m and make sure you hit the correct key.']);
    end
