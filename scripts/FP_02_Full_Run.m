...
%input files
 ...
 %"'subid'fp_sample.txt-calpoly"--> calibration parameters
%output file
...
caloutname=[respath,subid,'fp_sample.txt'];
...
[a1,trigger_time,trigger]=textread([respath,subid,'fp_trigger.txt'],'%s%f%s','headerlines',2);
...
trial_type=textread([respath,subid,'fp.out'],'%s');
s=length(trial_type);
...
for trial=1:trial_num
    ...
    if strcmp(trial_type(trial),'PSL10') logtmpnum=0; logtmp='PS1'; end
    if strcmp(trial_type(trial),'PSL05') logtmpnum=0; logtmp='PS2'; end
    if strcmp(trial_type(trial),'PSR05') logtmpnum=1; logtmp='PS3'; end
    if strcmp(trial_type(trial),'PSR10') logtmpnum=1; logtmp='PS4';end
...
figure(1)
    ...
    if key   
            ...
            text(-100,8,'Main reason for not scoring? <1=Anticipatory saccade; 2=Eye blinks at stimulis onset; 3=;no movement; 4=Other; 0=skip>');
